...
shell("openssl ecparam -name prime256v1 -out common/prime256v1.pem")
shell("openssl ecparam -in prime.pem -text")

print("- Generating ECDSA private and public keys\n")
shell("openssl genpkey -paramfile common/prime256v1.pem -out server/ecdsa_key.pem")
shell("openssl pkey -in server/ecdsa_key.pem -pubout -out common/serverkey.pem")
shell("openssl genpkey -paramfile common/prime256v1.pem -out CA/ecdsa_key.pem")
shell("openssl pkey -in CA/ecdsa_key.pem -pubout -out common/cakey.pem")
shell("openssl genpkey -paramfile common/prime256v1.pem -out MS/ecdsa_key.pem")
shell("openssl pkey -in MS/ecdsa_key.pem -pubout -out common/mskey.pem")

print("- Generating dsaparams\n")
#shell("openssl dsaparam -out common/dsaparam.pem 1024")
shell("cp ./dsaparam.pem ./common/dsaparam.pem")
shell("openssl dsaparam -in common/dsaparam.pem -text")

print("- Generating auto signed certification of root certification authority\n")
shell("openssl req -x509 -key CA/ecdsa_key.pem -days 3650 -out common/CAcert.pem -subj \"/C=IT/ST=LAZ/L=ROME/O=GOVERNO/CN=MINISTERO_DIGITALIZZAZIONE\"")
shell("openssl x509 -in CA/CAcrt.pem -text")
print("- Generating tree directory of root certification authority\n")
shell("mkdir CA/demoCA")
shell("cp common/CAcert.pem CA/demoCA/cacert.pem")
shell("touch CA/demoCA/index.txt")
shell("touch CA/demoCA/serial")
shell("echo \"00\" >> CA/demoCA/serial")
shell("mkdir CA/demoCA/private")
shell("cp CA/ecdsa_key.pem CA/demoCA/private/cakey.pem")
shell("mkdir CA/demoCA/newcerts")

print("- Generating certificate request of MS\n")
shell("openssl req -new -key MS/ecdsa_key.pem -out MS/requestMS.pem -subj \"/C=IT/ST=LAZ/L=ROME/O=GOVERNO/CN=MINISTERO_SALUTE\"")
#shell("openssl x509 -in CA/CAcrt.pem -text")
print("- Root certification authority sign certificate request of MS\n")
shell("cd CA; yes | openssl ca -in ../MS/requestMS.pem -out ../common/MScert.pem -policy policy_anything")
